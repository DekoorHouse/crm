 ...
 */
// buildAdvancedTemplatePayload se movió a server/whatsappTemplates.js (módulo compartido
// con el servicio de cobranza automática); se importa arriba. Lógica idéntica.
...
router.post('/envios/meta-purchase', async (req, res) => {
    try {
        const body = req.body || {};
        const { status = 200, ...result } = await sendOrderPurchase(body.docId, {
            source: body.automatic === true ? 'envios_auto' : 'envios_manual',
...
// respuesta es el mismo de siempre (la página public/cobranza lee success/skipped/reason).
router.post('/cobranza/enviar', async (req, res) => {
    try {
        ...
        const result = await cobrarContacto({ contactId, instructions, orderNumbers });
        res.json(result);
    } catch (error) {
