...
function receiptReviewState(order, receipt, related, amount) {
    ...
    if (!order.totalPending && amountCents + receivedCents > totalCents) risks.push({ code: 'over_total', message: 'Este abono haría que el importe recibido supere el total del pedido.' });
    ...
    const safetyToken = hash(JSON.stringify({ orderId: receipt.orderId, totalCents, receivedCents, totalPending: !!order.totalPending,
    ...
    return { safetyToken, totalPending: !!order.totalPending, orderNumber: `DH${order.consecutiveOrderNumber}`, amountCents, receivedCents, totalCents,
