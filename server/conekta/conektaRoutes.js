...
router.post('/webhook', async (req, res) => {
    try {
        ...
        if (event.type === 'order.paid') {
            ...
            // Auto-crear guía J&T si el envío es J&T y hay dirección
            if (addr && (!conektaData.shippingMethod || conektaData.shippingMethod === 'J&T Express') && jtService.isConfigured()) {
                try {
                    const jtResult = await jtService.createOrder({
                        orderNumber,
                        receiverName: conektaData.customerName,
                        receiverPhone: conektaData.customerPhone.replace(/^\+?52/, ''),
                        street: addr.street || '',
                        colonia: addr.colonia || '',
                        city: addr.city || '',
                        state: addr.state || '',
                        zip: addr.zip || '',
                        productName: conektaData.productName,
                    });
                    ...
                    if (jtResult.success) {
                        await db.collection('guias_jt').add({
                            ...
                            receiverName: conektaData.customerName,
                            receiverPhone: conektaData.customerPhone,
                            address: addressStr,
                            productName: conektaData.productName,
                            status: 'created',
                            autoGenerated: true,
                        ...
                        console.log(`[CONEKTA WEBHOOK] Guía J&T auto-creada: ${jtResult.waybillNo} para ${orderNumber}`);
                    } else {
                        console.warn(`[CONEKTA WEBHOOK] No se pudo crear guía J&T para ${orderNumber}: ${jtResult.message}`);
                    }
                } catch (jtError) {
                    console.error(`[CONEKTA WEBHOOK] Error auto-creando guía J&T para ${orderNumber}:`, jtError.message);
                }
            }
        }
