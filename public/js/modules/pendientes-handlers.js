...
function pendContactCard(c, col) {
    ...
    if (col.startsWith('pago_')) {
        ...
        const motivo = c.deliveryIncidentPending ? 'Entrega en disputa: contrastar dirección, guía y evidencia de recepción. Sin responsabilidad confirmada. Reporte del cliente: ' + (c.deliveryIncidentReport || '') : c.mediaDeliveryPending
    ...
    try {
        ...
        _pendPushUndo(_pendRemove('atencion', contactId), `${card.name || contactId}: marcada como atendida`,
            async () => {
                ...
                await _pendPost(`pendientes/atencion/${contactId}/reabrir`, { reason: card.reason || null, at: card.at || null, mediaDeliveryPending: card.mediaDeliveryPending === true, deliveryIncidentPending: card.deliveryIncidentPending === true });
