...
// --- START: NEW ORDER REGISTRATION HANDLERS ---
function handleOpenRegisterOrderModal() {
    openRegisterOrderModal();
}

async function handleSaveNewOrderFromCRM(event) {
    event.preventDefault();
    const errorMessageEl = document.getElementById('crm-mensajeErrorPedido');
    errorMessageEl.textContent = '';

    let productoFinal = document.getElementById('crm-pedidoProductoSelect').value;
    const productoOtroInput = document.getElementById('crm-pedidoProductoOtro');
    if (productoFinal === 'Otro') {
        productoFinal = productoOtroInput.value.trim();
        if (!productoFinal) {
            errorMessageEl.textContent = '¡El nombre del producto (Otro) es obligatorio!';
            productoOtroInput.focus();
            return;
        }
    }

    const telefono = document.getElementById('crm-pedidoTelefono').value.trim();
    if (!telefono) {
        errorMessageEl.textContent = '¡El número de teléfono es obligatorio!';
        document.getElementById('crm-pedidoTelefono').focus();
        return;
    }

    if (!auth.currentUser) {
        errorMessageEl.textContent = 'Necesitas iniciar sesión para guardar pedidos.';
        return;
    }
    
    const guardarBtn = document.getElementById('crm-btnGuardarPedido');
    const cancelarBtn = document.getElementById('crm-btnCancelarPedido');
    ...
    try {
        const uploadPhotos = async (manager, storagePath) => {
        ...
        guardarBtn.innerHTML = '<i class="fas fa-cloud-upload-alt"></i> Subiendo fotos...';
        const finalOrderPhotoUrls = await uploadPhotos(state.crmOrderPhotosManager, 'pedidos');
        const finalPromoPhotoUrls = await uploadPhotos(state.crmPromoPhotosManager, 'promociones');
        
        guardarBtn.innerHTML = '<i class="fas fa-database"></i> Guardando...';

        const email = auth.currentUser.email;
        const name = email.split('@')[0];
        const vendedor = name.charAt(0).toUpperCase() + name.slice(1);
        ...
        const pedidoData = {
            ...
            comentarios: document.getElementById('crm-pedidoComentarios').value.trim(),
            datosProducto: document.getElementById('crm-pedidoDatosProducto').value.trim(),
            datosPromocion: document.getElementById('crm-pedidoDatosPromocion').value.trim(),
            precio: Number(document.getElementById('crm-pedidoPrecio').value) || 0,
        };
        ...
        showCrmOrderConfirmationModal(result.newOrderNumber);

    } catch (error) {
        errorMessageEl.textContent = `Error: ${error.message}. Intenta de nuevo.`;
        console.error("Error guardando pedido desde CRM:", error);
    } finally {
...
// --- END: NEW ORDER REGISTRATION HANDLERS ---
