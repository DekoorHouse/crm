...
export default function ContactDetails({ contact, onClose, onNewOrder, onStatusChange }: ContactDetailsProps) {
  ...
  async function handlePedirDatosMty() {
    setShowConfirmMty(false);
    const promise = pedirDatosMty(contact.id);
    toast.promise(promise, {
      loading: "Enviando solicitud de datos MTY...",
      success: (data) => `Enlace MTY enviado para pedido ${data.orderNumber}`,
      error: (err) => err instanceof Error ? err.message : "Error",
    });
  }

  async function handlePedirDatosDgo() {
    setShowConfirmDgo(false);
    const promise = pedirDatosDgo(contact.id);
    toast.promise(promise, {
      loading: "Enviando solicitud de datos DGO...",
      success: (data) => `Enlace DGO enviado para pedido ${data.orderNumber}`,
      error: (err) => err instanceof Error ? err.message : "Error",
    });
  }
    ...
    <aside className="fixed md:static inset-0 md:inset-auto z-40 md:z-auto w-full md:w-80 h-full flex flex-col border-l border-outline-variant/15 bg-surface-container-lowest flex-shrink-0">
      ...
      <div className="px-3 py-3 border-t border-outline-variant/10 space-y-1.5">
        ...
        <button onClick={() => setShowConfirmMty(true)}
          ...
          Pedir Datos de Envío (MTY)
        </button>
        <button onClick={() => setShowConfirmDgo(true)}
          disabled={orders.length === 0}
          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold bg-secondary-container text-on-secondary-container hover:opacity-90 transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-sm">
          <span className="material-symbols-outlined" style={{ fontSize: 16 }}>two_wheeler</span>
          Pedir Datos de Envío (DGO)
        </button>
        {onNewOrder && (
      ...
      {showConfirmMty && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/40" onClick={() => setShowConfirmMty(false)}>
          <div className="bg-surface-container-lowest rounded-2xl shadow-2xl border border-outline-variant/15 w-80 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="px-5 pt-5 pb-3">
              <div className="w-10 h-10 rounded-full bg-secondary-container flex items-center justify-center mx-auto mb-3">
                <span className="material-symbols-outlined text-on-secondary-container" style={{ fontSize: 22 }}>pin_drop</span>
              </div>
              <h4 className="text-sm font-bold text-on-surface text-center">Pedir datos de envio (MTY)</h4>
              <p className="text-xs text-on-surface-variant text-center mt-1.5 leading-relaxed">
                Se enviara al cliente el enlace del formulario de entrega local en Monterrey para su ultimo pedido.
              </p>
            </div>
            <div className="flex gap-2 px-5 pb-5 pt-2">
              <button
                onClick={() => setShowConfirmMty(false)}
                className="flex-1 py-2 rounded-xl text-xs font-bold bg-surface-container-low text-on-surface-variant hover:bg-surface-container-high transition-colors"
              >
                Cancelar
              </button>
              <button
                onClick={handlePedirDatosMty}
                className="flex-1 py-2 rounded-xl text-xs font-bold bg-primary text-on-primary hover:opacity-90 transition-all shadow-sm"
              >
                Enviar
              </button>
      ...
      {/* Confirm modal — Pedir Datos de Envío (DGO) */}
      {showConfirmDgo && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/40" onClick={() => setShowConfirmDgo(false)}>
          <div className="bg-surface-container-lowest rounded-2xl shadow-2xl border border-outline-variant/15 w-80 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="px-5 pt-5 pb-3">
              <div className="w-10 h-10 rounded-full bg-secondary-container flex items-center justify-center mx-auto mb-3">
                <span className="material-symbols-outlined text-on-secondary-container" style={{ fontSize: 22 }}>two_wheeler</span>
              </div>
              <h4 className="text-sm font-bold text-on-surface text-center">Pedir datos de envio (DGO)</h4>
              <p className="text-xs text-on-surface-variant text-center mt-1.5 leading-relaxed">
                Se enviara al cliente el enlace del formulario de entrega local en Durango para su ultimo pedido.
              </p>
            </div>
            <div className="flex gap-2 px-5 pb-5 pt-2">
              <button
                onClick={() => setShowConfirmDgo(false)}
                className="flex-1 py-2 rounded-xl text-xs font-bold bg-surface-container-low text-on-surface-variant hover:bg-surface-container-high transition-colors"
              >
                Cancelar
              </button>
              <button
                onClick={handlePedirDatosDgo}
                className="flex-1 py-2 rounded-xl text-xs font-bold bg-primary text-on-primary hover:opacity-90 transition-all shadow-sm"
              >
                Enviar
              </button>
