...
test('the lamp frame keeps its real cutting size and is a 0.3 mm cutting line', () => {
    const frame = presetObject('lamp-frame', { x: 100, y: 150 }, '#ff0000');
    ...
    // Without an outline colour for new objects it still gets a visible line.
    assert.equal(presetObject('lamp-frame', { x: 0, y: 0 }, 'none').stroke, '#000000');
