// Firestore contract used by cut-job recovery tests: atomic commits, exclusive transactions,
// create conflicts and injected storage failures. No credentials or network.
module.exports = function svgCutFirestore() {
    const rows = new Map();
    ...
    function mergeFields(base, values) {
        for (const [key, value] of Object.entries(values)) {
            const parts = key.split('.'); let target = base;
            for (const k of parts.slice(0, -1)) target = target[k] ||= {};
            const k = parts[parts.length - 1];
            if (value === undefined) delete target[k]; else target[k] = copy(value);
        }
        return base;
    }
    function apply(writes) {
        ...
        for (const [op, ref, data, opts] of writes) rows.set(ref.path, copy(op === 'update' || opts?.merge ? mergeFields(copy(rows.get(ref.path) || {}), data) : data));
    }
    function doc(path) {
    ...
    function query(path, filters = [], sort = null, limit = Infinity) {
        return { doc: id => doc(path + '/' + id),
            ...
            get: async () => {
                ...
                docs = docs.filter(d => filters.every(([k, op, val]) => op === 'in' ? val.includes(get(d.data(), k)) : get(d.data(), k) === val));
